ParseBuddy = {
    ...
    State = {
        ...
        EvaluateBoss = function()
            return {
                {
                    group = { label = "Spell Vulnerability", missingText = "CoE / CoS", spells = { { spellIds = { 1 } } } },
                    state = "active",
                ...
                {
                    group = { label = "Judgement", missingText = "Wisdom / Light", spells = { { spellIds = { 1 } } } },
                    state = "missing",
                },
            }
        end,
...
ParseBuddyDB = { warningCapSeconds = 10, pullGracePeriod = 6 }
GetTime = function()
    return 100
end
time = function()
...
-- Load the real row mapper rather than stubbing it. /pb dump, completed
-- snapshots and the retained live view are all formatted through it, so a
-- stub here would not catch the mapper dropping a field the dump depends on.
local uiStub = ParseBuddy.UI
assert(loadfile("DebuffLibrary.lua"))()
assert(loadfile("UI.lua"))()
ParseBuddy.UI.mode = uiStub.mode
ParseBuddy.UI.frame = uiStub.frame
...
ParseBuddy.State.EvaluateBoss = function()
    return {
        { group = { label = "Spell Vulnerability", missingText = "CoE / CoS", spells = { { spellIds = { 1 } } } }, state = "missing" },
    }
end
...
ParseBuddy.State.EvaluateBoss = function()
    return {
        {
            group = { label = "Faerie Fire", missingText = "Faerie Fire", spells = { { spellIds = { 1 } } } },
            state = "active",
...
ParseBuddy.State.EvaluateBoss = function()
    return {
        { group = { label = "Judgement", missingText = "Wisdom / Light", spells = { { spellIds = { 1 } } } }, state = "missing" },
    }
end
