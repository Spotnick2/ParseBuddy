...
- A row carrying a live effect renders on one compact line; a row reporting a missing, pending, unavailable, or disabled group renders on two, naming the group and what would satisfy it. State shows as a left accent on a dark row, never as a full-row fill. The expiring window scales to each effect's own duration rather than a fixed number of seconds.
- Every group has a visibility tier. `always` reports both presence and absence; `applied` draws a row only while the effect is actually on the boss and never reports it missing. An `applied` group overrides both display modes, so it is visible when healthy-active in Problems Only and hidden when missing in Full List. `applied` groups never alert, whatever their requirement flag says.
- Live display filtering is a UI concern. For `always` groups, Problems Only hides healthy active rows and shows required missing/grace, partial, expiring, and unknown-source rows; Full List shows every enabled `always` group. Test mode and diagnostics remain unfiltered.
...
- A group whose provider is talent-gated rather than class-gated ships as `applied`, so class presence never produces a missing claim or an alert for an effect nobody specced.
...
- Sounds, assignments, provider whispers, talent/spec inspection, import/export, named profile management, and player scoring remain deferred.
