...
The completed MVP implementation line is `0.1.7.0`. Follow-up acceptance fixes increment PATCH: persisted frame opacity shipped in `0.1.7.1`, the bounded hybrid diagnostic snapshot shipped in `0.1.7.2`, neutral encounter-target fallback support shipped in `0.1.7.3`, the lock control and Curse of Recklessness group shipped in `0.1.7.4`, completed-snapshot live-state preservation shipped in `0.1.7.5`, live display modes shipped in `0.1.7.6`, global/personal group settings shipped in `0.1.7.7`, in-memory encounter uptime summaries shipped in `0.1.7.8`, the numeric multi-boss target registry shipped in `0.1.7.9`, cached roster-aware debuff availability shipped in `0.1.7.10`, optional missing-debuff broadcasts shipped in `0.1.7.11`, and the static configuration UX prototype shipped in `0.1.7.12`.
